...
//*****************************************************************************
//
// touch_tm4c1294xl.c - Touch screen driver for the EK-TM4C1294XL board.
//
//*****************************************************************************
...
#include "Kentec320x240x16_touch_tm4c1294xl.h"
...
#define BOOSTERPACK_2
#ifdef BOOSTERPACK_1
	#define TS_P_XN_PERIPH          SYSCTL_PERIPH_GPIOL
	#define TS_P_XP_PERIPH          SYSCTL_PERIPH_GPIOE
	#define TS_P_YP_PERIPH          SYSCTL_PERIPH_GPIOE
	#define TS_P_YN_PERIPH          SYSCTL_PERIPH_GPIOP

	#define TS_P_XN_BASE            GPIO_PORTL_BASE
	#define TS_P_XP_BASE            GPIO_PORTE_BASE
	#define TS_P_YP_BASE            GPIO_PORTE_BASE
	#define TS_P_YN_BASE            GPIO_PORTP_BASE
	...
	#define TS_XP_ADC               ADC_CTL_CH2
	#define TS_YP_ADC               ADC_CTL_CH3
#else
	#define TS_P_XN_PERIPH          SYSCTL_PERIPH_GPIOK
	#define TS_P_XP_PERIPH          SYSCTL_PERIPH_GPIOB
	#define TS_P_YP_PERIPH          SYSCTL_PERIPH_GPIOB
	#define TS_P_YN_PERIPH          SYSCTL_PERIPH_GPIOM

	#define TS_P_XN_BASE            GPIO_PORTK_BASE
	#define TS_P_XP_BASE            GPIO_PORTB_BASE
	#define TS_P_YP_BASE            GPIO_PORTB_BASE
	#define TS_P_YN_BASE            GPIO_PORTM_BASE

	#define TS_XN_PIN               GPIO_PIN_7//L
	#define TS_XP_PIN               GPIO_PIN_5//R
	#define TS_YP_PIN               GPIO_PIN_4//U
...
TouchScreenIntHandler(void)
{
    ...
    switch(g_ui32TSState)
    {
        ...
        case TS_STATE_SKIP_X:
        {
            ...
            //
            HWREG(TS_P_YP_BASE + GPIO_O_AMSEL) =
                  HWREG(TS_P_YP_BASE + GPIO_O_AMSEL) | TS_YP_PIN;

            //
            // Configure the Y-axis touch layer pins as inputs.
            //
            HWREG(TS_P_YP_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_YP_BASE + GPIO_O_DIR) & ~TS_YP_PIN;
            HWREG(TS_P_YN_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_YN_BASE + GPIO_O_DIR) & ~TS_YN_PIN;
        ...
        case TS_STATE_READ_X:
        {
            ...
            //
            HWREG(TS_P_YP_BASE + GPIO_O_DIR) =
                HWREG(TS_P_YP_BASE + GPIO_O_DIR) | TS_XP_PIN | TS_YP_PIN;
            HWREG(TS_P_XN_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_XN_BASE + GPIO_O_DIR) | TS_XN_PIN;
            HWREG(TS_P_YN_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_YN_BASE + GPIO_O_DIR) | TS_YN_PIN;
            ...
            //
            HWREG(TS_P_XN_BASE + GPIO_O_DATA + (TS_XN_PIN << 2)) = 0;
            HWREG(TS_P_YN_BASE + GPIO_O_DATA + (TS_YN_PIN << 2)) = 0;
            HWREG(TS_P_YP_BASE + GPIO_O_DATA + ((TS_XP_PIN | TS_YP_PIN) << 2)) =
                TS_YP_PIN;
        ...
        case TS_STATE_SKIP_Y:
        {
            ...
            //
            HWREG(TS_P_XP_BASE + GPIO_O_AMSEL) =
                  HWREG(TS_P_XP_BASE + GPIO_O_AMSEL) | TS_XP_PIN;

            //
            // Configure the X-axis touch layer pins as inputs.
            //
            HWREG(TS_P_XP_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_XP_BASE + GPIO_O_DIR) & ~TS_XP_PIN;
            HWREG(TS_P_XN_BASE + GPIO_O_DIR) =
                  HWREG(TS_P_XN_BASE + GPIO_O_DIR) & ~TS_XN_PIN;
...
TouchScreenInit(uint32_t ui32SysClock)
{
    ...
    SysCtlPeripheralEnable(TS_P_XP_PERIPH);
    SysCtlPeripheralEnable(TS_P_YP_PERIPH);
    SysCtlPeripheralEnable(TS_P_XN_PERIPH);
    SysCtlPeripheralEnable(TS_P_YN_PERIPH);
    ...
    //
    GPIOPinTypeGPIOOutput(TS_P_YP_BASE, TS_XP_PIN | TS_YP_PIN);
    GPIOPinTypeGPIOOutput(TS_P_XN_BASE, TS_XN_PIN);
    GPIOPinTypeGPIOOutput(TS_P_YN_BASE, TS_YN_PIN);

    GPIOPinWrite(TS_P_YP_BASE, TS_XP_PIN | TS_YP_PIN, 0x00);
    GPIOPinWrite(TS_P_XN_BASE, TS_XN_PIN, 0x00);
    GPIOPinWrite(TS_P_YN_BASE, TS_YN_PIN, 0x00);
    ...
    if((HWREG(TIMER2_BASE + TIMER_O_CTL) & TIMER_CTL_TAEN) == 0)
    {
        ...
        TimerLoadSet(TIMER2_BASE, TIMER_A, (ui32SysClock / 400) - 1);
